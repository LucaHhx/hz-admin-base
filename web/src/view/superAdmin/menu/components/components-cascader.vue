<template>
  <div class="flex justify-between items-center gap-2 w-full">
    <el-cascader
      ...
      :placeholder="$t('superAdmin.menuComponent.selectFilePath')"
      :options="pathOptions"
      filterable
      class="w-full"
      clearable
    ...
    <el-input
      v-else
      v-model="tempPath"
      :placeholder="$t('superAdmin.menuComponent.inputPathPlaceholder')"
      @change="emitChange"
    />
    <el-button @click="togglePathIsSelect">
      {{ pathIsSelect ? $t('superAdmin.menuComponent.manualInput') : $t('superAdmin.menuComponent.quickSelect') }}
    </el-button>
  </div>
</template>
