  ...
  <div>
    <div class="sticky top-0.5 z-10 flex space-x-2">
      <el-input
        ...
        class="flex-1"
        :placeholder="$t('superAdmin.authority.components.apis.filterName')"
      />
      <el-input
        v-model="filterTextPath"
        class="flex-1"
        :placeholder="$t('superAdmin.authority.components.apis.filterPath')"
      />
      <el-button
        class="float-right"
        type="primary"
...
const buildApiTree = (apis) => {
  ...
  apis &&
      apis.forEach((item) => {
        // console.log(item)
        item.description = t('apis.' + item.path)
  ...
  for (const key in apiObj) {
    const treeNode = {
      ID: key,
      description: t('menu.' + key) + t('superAdmin.authority.components.apis.group'),
      children: apiObj[key]
    }
