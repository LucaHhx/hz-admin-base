<template>
  <div>
    <SelectTool :filter-id="1" :value="value" />
    <DivSearch ref="searchRef" :columns="columns">
      <template #elFormItem>
        <el-form-item>
    ...
    <div class="hab-table-box">
      <ThreeColumnLayout :section-gap="5" :item-gap="10">
        <template #left>
          ...
          <el-button
            v-auth="btnAuth.batchDelete" :icon="Delete" type="danger" plain
            :disabled="!tableRef?.getSelectedData().length" @click="onDelete"
          >
        ...
        <template #right>
          <ColOptions v-if="btnAuth.columnConfig" :columns="columns" :struct-name="structName" />
          <CustomExport
            :fetch-function="getSysDataFilterList" :search-info="tableRef?.getQueryInfo()"
            :columns="columns" :file-name="structName" :batch-size="1000"
          />
          <CustomImport
            :import-function="importSysDataFilter" :columns="columns" style="margin-top: 10px;"
    ...
    <div class="hab-table-box">
      ...
      >
        <template v-if="btnAuth.info || btnAuth.edit || btnAuth.delete" #operate="scope">
          ...
          <el-button
            v-auth="btnAuth.edit" type="primary" link class="table-button"
            @click="updateSysDataFilterFunc(scope.row)"
          >
            <el-icon class="button-icon">
              <Edit />
            </el-icon> {{ $t('common.edit') }}
          </el-button>
          <el-button
            v-auth="btnAuth.delete" type="danger" link class="table-button"
            @click="deleteSysDataFilterFunc(scope.row)"
          >
            <el-icon class="button-icon">
              <Delete />
    ...
    <SysDataFilterForm ref="sysDataFilterFormRef" :callback="getTableData" />
  </div>
</template>
...
import SysDataFilterForm from './sysDataFilterForm.vue'
...
import {
  deleteSysDataFilter,
  deleteSysDataFilterByIds,
  getSysDataFilterList,
  importSysDataFilter
} from '@/api/system/sysDataFilter'

// const { t } = useI18n()

const structName = 'sysDataFilter'
defineOptions({
  name: 'SysDataFilter'
})
const value = ref(null)
const sysDataFilterFormRef = ref(null)
const columns = ref([])
...
const getTableData = async() => {
  ...
  tableData.value = []
  const table = await getSysDataFilterList(searchInfo)
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('Are you sure you want to delete?', 'Prompt', {
    ...
    const res = await deleteSysDataFilterByIds({ IDs })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
...
// 更新行
const updateSysDataFilterFunc = async(row) => {
  sysDataFilterFormRef?.value?.openDialog('edit', row)
}

// 删除行
const deleteSysDataFilterFunc = async(row) => {
  ElMessageBox.confirm('Are you sure you want to delete?', 'Prompt', {
    confirmButtonText: 'Confirm',
    cancelButtonText: 'Cancel',
    type: 'warning'
  }).then(async() => {
    const res = await deleteSysDataFilter({ ID: row.ID })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: 'Deletion successful'
...
// 打开弹窗
const openDialog = () => {
  sysDataFilterFormRef?.value?.openDialog('create', null)
}

// 打开详情
const getDetails = async(row) => {
  sysDataFilterFormRef?.value?.openDialog('view', row)
}
