			...
			for j := 0; j < len(secondDirs); j++ {
				...
				case "translation":
					...
					for k := 0; k < len(threeDirs); k++ {
						...
						// 处理国际化翻译文件
						hasZhCn := strings.Index(threeDirs[k].Name(), "zh-CN")
						hasEnUs := strings.Index(threeDirs[k].Name(), "en-US")

						if hasZhCn == -1 && hasEnUs == -1 {
							return nil, nil, nil, errors.Errorf("[filpath:%s]非法翻译模版文件!", four)
						}
