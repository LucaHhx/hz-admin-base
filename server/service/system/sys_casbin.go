...
func (casbinService *CasbinService) UpdateCasbin(adminAuthorityID, AuthorityID uint, casbinInfos []request.CasbinInfo) error {
	...
	if global.HAB_CONFIG.System.UseStrictAuth {
		...
		casbinInfos = lo.Filter(casbinInfos, func(item request.CasbinInfo, index int) bool {
			for j := range apis {
				if apis[j].Path == item.Path && apis[j].Method == item.Method {
					return true
				}
			}
			return false
...
func (casbinService *CasbinService) UpdateCasbinApi(oldPath string, newPath string, oldMethod string, newMethod string) error {
	err := global.HAB_DB.Model(&gormadapter.CasbinRule{}).Where("v1 = ? AND v2 = ?", oldPath, oldMethod).Updates(map[string]interface{}{
		"v1": newPath,
		"v2": newMethod,
	}).Error
	if err != nil {
...
func (casbinService *CasbinService) Casbin() *casbin.SyncedCachedEnforcer {
	once.Do(func() {
		a, err := gormadapter.NewAdapterByDB(global.HAB_DB)
		if err != nil {
