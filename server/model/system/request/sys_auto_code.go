...
func (r *AutoCode) Apis() []model.SysApi {
	return []model.SysApi{
		...
		{
			Path:        "/" + r.Abbreviation + "/" + "import" + r.StructName,
			Description: "import",
			ApiGroup:    r.Abbreviation,
			Method:      "POST",
		},
...
func (r *AutoCode) Columns(meunId uint) []system.SysTableColumns {
	columns := make([]system.SysTableColumns, 0)
	for i, field := range r.Fields {
		column := system.SysTableColumns{
			TbName:     r.TableName,
			StructName: r.PackageName,
		...
		case "enum":
			column.With = 100
			column.Enum = []string{"1", "2", "3"} // 默认枚举值
		case "int", "int32":
			column.With = 80
		case "int64", "float64":
			column.With = 100
		...
		case "string", "richtext":
			column.DefaultFilter = "like"
		case "bool", "enum":
			column.DefaultFilter = "="
		case "int", "int32", "int64", "float64":
			column.DefaultFilter = "="
		case "datetime", "date":
			column.DefaultFilter = "between"
		case "json", "array", "binary":
			column.DefaultFilter = "like"
		default:
			column.DefaultFilter = "="
		}
		...
		switch field.FieldType {
		case "string", "richtext":
			column.FilterList = []string{"=", "!=", "like", "in", "not in"}
		case "bool", "enum":
			column.FilterList = []string{"=", "!="}
		case "int", "int32", "int64", "float64":
			column.FilterList = []string{"=", "!=", ">", ">=", "<", "<=", "in", "not in", "between", "not between"}
		case "datetime", "date":
			column.FilterList = []string{"=", "!=", ">", ">=", "<", "<=", "between", "not between"}
		case "json", "array":
			column.FilterList = []string{"=", "!=", "like"}
		case "binary":
			column.FilterList = []string{"=", "!="}
		default:
			column.FilterList = []string{"=", "!=", "like"}
	...
	if r.GvaModel {
		...
		columns = append(columns, system.SysTableColumns{
			TbName:        r.TableName,
			StructName:    r.PackageName,
			ColumnName:    "id",
			JsonName:      "ID",
			...
			Filter:        true,
			DefaultFilter: "=",
			FilterList:    []string{"=", "!=", ">", ">=", "<", "<=", "in", "not in", "between", "not between"},
			FormInfo: system.FormInfo{
				FormWith:     45,
				FormOrder:    0,
				FormDisabled: true,
		...
		columns = append(columns, system.SysTableColumns{
			...
			FilterList:    []string{"=", "!=", ">", ">=", "<", "<=", "between", "not between"},
			FormInfo: system.FormInfo{
				FormWith:     45,
				FormOrder:    len(columns),
				FormDisabled: true,
				FormHidden:   true,
			},
		})
...
func (r *AutoCode) Pretreatment() error {
	r.Module = global.HAB_CONFIG.AutoCode.Module
