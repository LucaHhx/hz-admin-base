...
func (a *AuthorityMenuApi) GetBaseMenuTree(c *gin.Context) {
	...
	menus, err := menuService.GetBaseMenuTree(authority, tep, pas)
	if err != nil {
		global.HAB_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("Failed to get", c)
		return
...
func (a *AuthorityMenuApi) GetOwnMenuList(c *gin.Context) {
	authority := utils.GetUserAuthorityId(c)
	tep, pas := utils.GetUserTypeInfo(c)
	menus, err := menuService.GetBaseMenuTree(authority, tep, pas)
	if err != nil {
...
func (a *AuthorityMenuApi) AddMenuAuthority(c *gin.Context) {
	...
	menu := lo.Filter(authorityMenu.Menus, func(item system.SysBaseMenu, index int) bool {
		return item.ID < 15023
	})
	buttons := lo.Filter(authorityMenu.Menus, func(item system.SysBaseMenu, index int) bool {
		return item.ID >= 15023
	})
	butGroup := lo.GroupBy(buttons, func(item system.SysBaseMenu) uint {
		return item.ParentId
	})
	for u, menus := range butGroup {
		err = authorityBtnService.SetAuthorityBtn(systemReq.SysAuthorityBtnReq{
			MenuID:      u,
			AuthorityId: authorityMenu.AuthorityId,
			Selected: lo.Map(menus, func(item system.SysBaseMenu, index int) uint {
				return item.ID - 15023
			}),
		})
		if err != nil {
			response.FailWithMessageData("添加失败!", err.Error(), c)
			return
		}
	}
	if err := menuService.AddMenuAuthority(menu, adminAuthorityID, authorityMenu.AuthorityId); err != nil {
		global.HAB_LOG.Error("添加失败!", zap.Error(err))
...
func (a *AuthorityMenuApi) GetMenuAuthority(c *gin.Context) {
	...
	if err != nil {
		global.HAB_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithDetailed(systemRes.SysMenusResponse{Menus: menus}, "Failed to get", c)
		return
	}
...
func (a *AuthorityMenuApi) GetMenuList(c *gin.Context) {
	...
	tep, pas := utils.GetUserTypeInfo(c)
	menuList, err := menuService.GetInfoList(authorityID, tep, pas)
	if err != nil {
		global.HAB_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("Failed to get", c)
		return
