...
func (m *mongo) Initialization() error {
	...
	if global.HAB_CONFIG.Mongo.IsZap {
		opts = internal.Mongo.GetClientOptions()
	}
	ctx := context.Background()
	client, err := qmgo.Open(ctx, &qmgo.Config{
		Uri:              global.HAB_CONFIG.Mongo.Uri(),
		Coll:             global.HAB_CONFIG.Mongo.Coll,
		Database:         global.HAB_CONFIG.Mongo.Database,
		MinPoolSize:      &global.HAB_CONFIG.Mongo.MinPoolSize,
		MaxPoolSize:      &global.HAB_CONFIG.Mongo.MaxPoolSize,
		SocketTimeoutMS:  &global.HAB_CONFIG.Mongo.SocketTimeoutMs,
		ConnectTimeoutMS: &global.HAB_CONFIG.Mongo.ConnectTimeoutMs,
		Auth: &qmgo.Credential{
			Username:   global.HAB_CONFIG.Mongo.Username,
			Password:   global.HAB_CONFIG.Mongo.Password,
			AuthSource: global.HAB_CONFIG.Mongo.AuthSource,
		},
	}, opts...)
	if err != nil {
		return errors.Wrap(err, "链接mongodb数据库失败!")
	}
	global.HAB_MONGO = client
	err = m.Indexes(ctx)
	if err != nil {
		return err
	}
...
func (m *mongo) CreateIndexes(ctx context.Context, name string, indexes [][]string) error {
	collection, err := global.HAB_MONGO.Database.Collection(name).CloneCollection()
	if err != nil {
	...
	for k1, v1 := range indexMap1 {
		...
		if len(fmt.Sprintf("%s.%s.$%s", collection.Name(), name, v1)) > 127 {
			err = global.HAB_MONGO.Database.Collection(name).CreateOneIndex(ctx, options.IndexModel{
				Key:          v1,
